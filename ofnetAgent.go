...
const (
	...
	CONNTRACK_TBL_ID        = 5
	CONNTRACK_STATE_TBL_ID  = 6
	EGRESS_SELECT_TBL_ID    = 7
	DST_GRP_TBL_ID          = EGRESS_TIER0_TBL_ID
	POLICY_TBL_ID           = EGRESS_TIER0_TBL_ID
	EGRESS_TIER0_TBL_ID     = 10
	EGRESS_TIER1_TBL_ID     = 11
	EGRESS_TIER2_TBL_ID     = 12
	INGRESS_SELECT_TBL_ID   = 20
	INGRESS_TIER0_TBL_ID    = 30
	INGRESS_TIER1_TBL_ID    = 31
	INGRESS_TIER2_TBL_ID    = 32
	SRV_PROXY_SNAT_TBL_ID   = 40
	IP_TBL_ID               = 41
	HOST_SNAT_TBL_ID        = 42
	CONNTRACK_COMMIT_TBL_ID = 45
	MAC_DEST_TBL_ID         = 46
)
...
func (self *OfnetAgent) AddLocalEndpoint(endpoint EndpointInfo) error {
	...
	if self.dpName == "vlanArpLearner" {
		...
		if err != nil {
			log.Errorf("Failed to add local endpoint (%+v) to datapath. Err: %v", ofnetEndpoint, err)
			return err
		}
...
func (self *OfnetAgent) RemoveLocalEndpoint(portNo uint32) error {
	...
	if self.dpName == "vlanArpLearner" {
		...
		if endpointObj == nil {
			err := fmt.Errorf("Deleting non-existing local endpoint for ofport: %+v", portNo)
			log.Error(err)
			return err
		}
		...
		if err != nil {
			log.Errorf("Failed to delete local endpoint: %+v. error: %v", *ofnetEndpoint, err)
		}
